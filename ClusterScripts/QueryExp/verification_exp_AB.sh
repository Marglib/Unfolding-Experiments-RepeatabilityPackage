...
for t in ${properties[@]} ; do
    for i in $(ls $F ) ; do
        ...
        sbatch --mail-user=$(whoami)  -n 1 -c $N --mem="${M}G"  ./run_part_queries.sh "$BIN -s DFS -n -x SLURM_ARRAY_TASK_ID $unzippedmodel $unzippedquery" "$ODIR/${i}.QSLURM_ARRAY_TASK_ID.${t}"  $M $TO $F $i $t
    done
done
...
for t in ${properties[@]} ; do
    for i in $(ls $F ) ; do
        ...
        unzippedmodel="temp/$t/$F/$i/$i-tp-unfolded.pnml"
        unzippedquery="temp/$t/$F/$i/$t-tp-unfolded.xml"
        sbatch --mail-user=$(whoami)  -n 1 -c $N --mem="${M}G"  ./run_part_queries.sh "$BIN -s DFS -ltl -n -x SLURM_ARRAY_TASK_ID $unzippedmodel $unzippedquery" "$ODIR/${i}.QSLURM_ARRAY_TASK_ID.${t}"  $M $TO $F $i $t
    done
done
